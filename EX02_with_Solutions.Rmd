...
## Step-by-Step Solution for Problem 1: Checking your working directory using `{here}`

### Step 1: Install and Load the `here` Package

```{r step1_code, eval=FALSE}
install.packages('here')
library(here)
```

First, we need to install and load the `here` package, which is used to manage file paths.

### Step 2: Check the Working Directory

```{r step2_code, eval=FALSE}
here()
```

Use the `here()` function to check the current working directory.




## Step-by-Step Solution for Problem 2: Loading libraries
...
First, install the required libraries `dplyr` and `ggplot2`.
...
Use the `library()` function to load the installed libraries.




## Step-by-Step Solution for Problem 3: Creating objects

### Step 1: Create a Numeric Object

```{r step1_code, eval=FALSE}
x <- 10
```

Create a numeric object `x` and assign the value 10 to it.

### Step 2: Create a Character Object

```{r step2_code, eval=FALSE}
y <- 'Hello'
```

Create a character object `y` and assign the string 'Hello' to it.
